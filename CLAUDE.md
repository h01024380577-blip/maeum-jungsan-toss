...
- Build output goes to `dist/` (not `.next/`) — configured via `distDir` in `next.config.ts`. The dev server deliberately uses a **separate** `dist-dev/`: `build:ait` and `deploy.sh` both start with `rm -rf dist`, which used to delete a running dev server's output and leave it serving ENOENT 500s on every request
